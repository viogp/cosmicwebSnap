...
float *pos_2_ngp(Catalog cat)
{
  ...
  float *grid=(float *)calloc(n_grid_tot,sizeof(float));
...
#pragma omp parallel default(none)		\
  ...
  {
    lint ii;
    float agrid=l_box/n_grid;
#pragma omp for nowait
    for(ii=0;ii<cat.np;ii++) {
      lint i0x,i0y,i0z;
      float xp=CLAMP(cat.pos[3*ii],0,l_box);
      float yp=CLAMP(cat.pos[3*ii+1],0,l_box);
      float zp=CLAMP(cat.pos[3*ii+2],0,l_box);
      i0x=(lint)(xp/agrid);
      i0y=(lint)(yp/agrid);
      i0z=(lint)(zp/agrid);
...
#pragma omp parallel default(none)		\
  ...
  {
    lint ii;
    float n_avg=(float)(cat.np)/n_grid_tot;
    float i_n_avg=1/n_avg;
...
#pragma omp parallel default(none)		\
  shared(cat,n_grid,n_grid_tot,grid,l_box)
  {
    lint ii;
    float agrid=l_box/n_grid;
    float ivcell=1./(agrid*agrid*agrid);
#pragma omp for nowait
    for(ii=0;ii<cat.np;ii++) {
      lint i0x,i0y,i0z;
      lint i1x,i1y,i1z;

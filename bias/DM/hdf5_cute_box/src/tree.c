...
static branch **mk_leaves(Catalog cat)
{
  ...
  for(ii=0;ii<nside*nside*nside;ii++) {
    if(leaves[ii]->np>0) {
      leaves[ii]->sons=malloc(3*leaves[ii]->np*sizeof(float));
      leaves[ii]->np=0;
    }
  }
  ...
  for(ii=0;ii<cat.np;ii++) {
    ...
    iz=(lint)(cat.pos[3*ii+2]/dx);
    index=iz+nside*(iy+nside*ix);
    ((float *)leaves[index]->sons)[3*leaves[index]->np]=
      cat.pos[3*ii];
    ((float *)leaves[index]->sons)[3*leaves[index]->np+1]=
      cat.pos[3*ii+1];
    ((float *)leaves[index]->sons)[3*leaves[index]->np+2]=
      cat.pos[3*ii+2];
    (leaves[index]->np)++;
  }
  ...
  for(ii=0;ii<nsidef;ii++) {
	...
	if(np<=max_tree_nparts) { //Make leaf from leaves
	  ...
	  if(np>0) { //Copy all particles to new leaf
	    brf[indexf]->sons=malloc(3*np*sizeof(float));
	    if(brf[indexf]->sons==NULL) error_mem_out();
	    for(ll=0;ll<8;ll++) {
	      memcpy((float *)(brf[indexf]->sons)+3*ipart,
		     (float *)(sons[ll]->sons),
		     3*(sons[ll]->np)*sizeof(float));
	      ipart+=sons[ll]->np;
	    }
	  }
...
branch *mk_tree(Catalog cat)
{
  ...
  float n_mean=(float)cat.np/pow(n_side_leaves,3);

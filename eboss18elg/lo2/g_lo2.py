...
lo2prop = np.zeros(shape=(len(lbins),len(pbins)))
lo2prop_ext = np.zeros(shape=(len(lbins),len(pbins)))
...
for ivol in range(nvol):
    ...
    if(os.path.isfile(gfile)):
        ...
        if (os.path.isfile(efile)):
            ...
            # lo2-prop
            H, xedges, yedges = np.histogram2d(lum,g,
                                               bins=[np.append(lbins,lmax),np.append(pbins,pmax)])
            lo2prop[:,:] = lo2prop[:,:] + H

            H, xedges, yedges = np.histogram2d(lum_ext,g_ext,
                                               bins=[np.append(lbins,lmax),np.append(pbins,pmax)])
...
print(min(r),max(r),min(r_ext),max(r_ext))
print(min(g),max(g),min(g_ext),max(g_ext))
print(min(rz),max(rz),min(rz_ext),max(rz_ext))
print(min(gr),max(gr),min(gr_ext),max(gr_ext))
...
ytit="g"
xmin=lmin ; xmax=lmax ; ymin=pmin ; ymax=pmax
ax.set_xlim(xmin,xmax) ; ax.set_ylim(ymin,ymax) 
ax.set_xlabel(xtit) ; ax.set_ylabel(ytit)

# Extincted
matplotlib.rcParams['contour.negative_linestyle'] = '-'
zz = np.zeros(shape=(len(lhist),len(phist))) 
ind = np.where(lo2prop_ext>0.)
zz = np.log10(pz) 
xx,yy = np.meshgrid(pbins,lbins)
al = [-3.5,-2.5,-1.5] 

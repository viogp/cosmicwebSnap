...
gsmf  = np.zeros(shape=(len(inleg),len(mbins)))
ssfrf = np.zeros(shape=(len(inleg),len(sbins)))
smf   = np.zeros(shape=(len(inleg),len(sbins),len(mbins)))
...
# sSFR-GSMF
H, xedges, yedges = np.histogram2d(ssfr,lmass,\
                                   bins=[np.append(sbins,smax),np.append(mbins,mmax)])
smf[index,:,:] = smf[index,:,:] + H
...
lsty = ['solid','dashed','dashed','solid','solid']
lwdt = [3.,1.5,1.5,1.5,2.]
for ii in range(len(inleg)):
